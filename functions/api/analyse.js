...
// ── Cookie auth helpers ──────────────────────────────────────────────────────
const COOKIE_NAME = 'gvt_auth';

/** Leest JWT uit httpOnly cookie; valt terug op Authorization header. */
const _getToken = (request) => {
  const cookieHeader = request.headers.get('Cookie') || '';
  const prefix = COOKIE_NAME + '=';
  const cookie = cookieHeader.split(';').map(s => s.trim()).find(s => s.startsWith(prefix));
  if (cookie) return decodeURIComponent(cookie.slice(prefix.length));
  // Fallback: Authorization header (voor backward compat tijdens transitie)
  const authHeader = request.headers.get('Authorization') || '';
  return authHeader.startsWith('Bearer ') ? authHeader.slice(7) : null;
};

/** Bouwt een Set-Cookie header voor de JWT. */
const _cookieHeader = (token) =>
  `${COOKIE_NAME}=${encodeURIComponent(token)}; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=28800`;

/** Wist de auth-cookie. */
const _clearCookieHeader = () =>
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    // ── Auth: JWT uit cookie (primair) of Authorization header (fallback) ──────
    let jwtPayload = null;
    if (!PUBLIEK_ACTIES.has(actie)) {
      const jwtToken = _getToken(request);
    ...
    if (actie === 'validateToken') {
      ...
      if (vernieuwd) {
        resHeaders['Set-Cookie'] = _cookieHeader(token);
      }
    ...
    if (actie === 'checkLogin') {
      ...
      if (code === adminCode) {
        ...
        return new Response(
          ...
          { headers: { ...cors, 'Set-Cookie': _cookieHeader(token) } }
        );
      }
    ...
    if (actie === 'setKlant') {
      const { naam, klanttype, email, sector } = body;
      // code wordt automatisch gegenereerd als unieke logincode als hij niet meegestuurd wordt
      const code = body.code || email?.split('@')[0].replace(/[^a-z0-9]/gi, '').toLowerCase() + Math.random().toString(36).slice(2, 6);
    ...
    if (actie === 'aanvraagMagicLink') {
      const { email } = body;
      if (!email) {
        return new Response(JSON.stringify({ error: { message: 'E-mailadres ontbreekt' } }), { headers: cors });
      }

      // Zoek klant op basis van email
      ...
      if (!klant) {
        // Geen foutmelding tonen — security best practice
        return new Response(JSON.stringify({ ok: true }), { headers: cors });
      }

      // Genereer een tijdelijk token (geldig 15 minuten)
    ...
    if (actie === 'verifyMagicLink') {
      const { token } = body;
      const payload = await jwtVerify(token, env);

      if (!payload || payload.type !== 'magiclink') {
        return new Response(
          JSON.stringify({ error: { message: 'Loginlink is ongeldig of verlopen' } }),
          { status: 401, headers: cors }
        );
      }

      // Haal klantdata op
      const klanten = await sb(env, `klanten?code=eq.${encodeURIComponent(payload.code)}`);
      const klant = Array.isArray(klanten) ? klanten[0] : null;

      if (!klant) {
        return new Response(
          JSON.stringify({ error: { message: 'Klant niet gevonden' } }),
          { status: 401, headers: cors }
        );
      }

      // Maak een volledige sessie-cookie aan
      const exp = Math.floor(Date.now() / 1000) + 8 * 3600;
      const sessieToken = await jwtSign({ type: 'klant', code: klant.code, naam: klant.naam, exp }, env);
      ...
      return new Response(
        ...
        { headers: { ...cors, 'Set-Cookie': _cookieHeader(sessieToken) } }
      );
    }
    ...
    if (actie === 'uitloggen') {
      return new Response(
        ...
        { headers: { ...cors, 'Set-Cookie': _clearCookieHeader() } }
      );
    }
